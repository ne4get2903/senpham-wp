...
if(!class_exists('Food_Plugin_Demo')){
	class Food_Plugin_Demo{
		function __construct(){
			...
			//$arr = explode('/',plugin_basename(__file__));
			//if($_GET['post_type'] == 'fpd-food'){
				//add_action('media_buttons','add_img1_button');
...
function fpd_save_food_meta($postID, $post){
	$metaKey = array();
	$metaKey['img1'] = 'img1';
	$metaKey['img2'] = 'img2';
	$metaKey['img3'] = 'img3';
	...
	if($count != 0){
		for($i =0; $i < $count; $i++){
			$nameIn[$i] = (isset( $_POST['nameIn'.($i+1)] ) ? sanitize_text_field( $_POST['nameIn'.($i+1)] ) : '' );
			$amountIn[$i] = (isset( $_POST['amount'.($i+1)] ) ? sanitize_text_field( $_POST['amount'.($i+1)] ) : '' );
		}
	}
